...
class ModelManager:
    ...
    def load_yolo(self):
        ...
        if name not in self.models:
            logger.info("Loading YOLOv11x-seg (Extra Large) model...")
    ...
    def load_xfeat(self):
        name = "xfeat"
        if name not in self.models:
            logger.info("Loading XFeat model...")
            self._ensure_vram_available(300.0)
            try:
                # Завантаження XFeat з PyTorch Hub
                model = torch.hub.load(
                    "verlab/accelerated_features", "XFeat", pretrained=True, top_k=2048
                )
        ...
        if name not in self.models:
            ...
            try:
                ...
                sp_config = {
                    ...
                    "max_num_keypoints": self.config.get("superpoint", {}).get(
                        "max_keypoints", 4096
                    ),
                }
    ...
    def load_lightglue(self):
        ...
        if name not in self.models:
            ...
            try:
                ...
                lg_config = {
                    "depth_confidence": self.config.get("lightglue", {}).get(
                        "depth_confidence", -1
                    ),
                    "width_confidence": self.config.get("lightglue", {}).get(
                        "width_confidence", -1
                    ),
                }
    ...
    def load_dinov2(self):
        name = "dinov2"
        if name not in self.models:
            logger.info("Loading DINOv2 (vitl14) Large model...")
            self._ensure_vram_available(1600.0)  # Large важить ~1.2-1.5 GB
            try:
                model = torch.hub.load("facebookresearch/dinov2", "dinov2_vitl14")
                model = model.eval().to(self.device)
                self.models[name] = model
                logger.success("DINOv2 Large model loaded successfully")
            except Exception as e:
