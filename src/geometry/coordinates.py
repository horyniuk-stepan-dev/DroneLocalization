...
class CoordinateConverter:
    ...
    def configure_projection(cls, mode: str, reference_gps: tuple = None):
        ...
        with cls._lock:
            ...
            if mode == 'WEB_MERCATOR':
                ...
                logger.warning("UTM configuration called without reference_gps. Initialization deferred.")
                cls._initialized = False
    ...
    def export_projection_metadata(cls) -> dict:
        ...
        return {
            "mode": cls._projection_mode,
            "reference_gps": cls._reference_gps
        }
    ...
    def _initialize_projection(cls, lat: float, lon: float):
        ...
        if cls._projection_mode == 'UTM':
            ...
            logger.info(f"Initialized UTM projection for zone {zone_number} based on ({ref_lat:.4f}, {ref_lon:.4f})")
        else:
        ...
        with CoordinateConverter._lock:
            if not CoordinateConverter._initialized:
                ...
                if CoordinateConverter._projection_mode == 'WEB_MERCATOR':
                    ...
                    raise RuntimeError("CoordinateConverter (UTM) must be configured with reference_gps before use.")
