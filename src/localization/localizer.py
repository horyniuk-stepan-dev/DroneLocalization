...
class Localizer:
        ...
        if (
            ...
            fb_inliers, fb_candidate_id, fb_mkpts_q, fb_mkpts_r, fb_rot, fb_total = (
                self._run_lightglue_fallback(
                    query_frame, static_mask, best_global_angle, best_global_candidates, height, width
                )
            )
    ...
    def _perform_local_matching(
        self, query_frame: np.ndarray, static_mask: np.ndarray | None, best_angle: int, candidates: list
        ...
        return best_inliers, best_candidate_id, best_mkpts_q_inliers, best_mkpts_r_inliers, best_total_matches

    def _run_lightglue_fallback(
        self, query_frame: np.ndarray, static_mask: np.ndarray | None, best_angle: int, candidates: list, height: int, width: int
    ) -> tuple[int, int, np.ndarray | None, np.ndarray | None, int, int]:
        """Запускає Fallback (SuperPoint+LightGlue)."""
        logger.info(
            f"Trying SuperPoint+LightGlue fallback on best angle {best_angle}°..."
        )
        k = best_angle // 90
        rotated_frame = np.rot90(query_frame, k=k).copy()
        rotated_mask = np.rot90(static_mask, k=k).copy() if static_mask is not None else None

        return self._try_lightglue_fallback(
            rotated_frame, rotated_mask, candidates, height, width
        )
    ...
    def _calculate_tracking_update(
        ...
        metric_centroid = GeometryTransforms.apply_affine(np.array([centroid_ref], dtype=np.float32), affine_ref)
        ...
        metric_offset = R_ref @ np.array([offset_rotated_base[0] * scale_x, offset_rotated_base[1] * scale_y], dtype=np.float32)
        ...
        gps_corners = self._calculate_fov_polygon(filtered_pt[0], filtered_pt[1], half_w, half_h, global_angle, R_ref)

        max_inliers = self.config.get("localization", {}).get("confidence_max_inliers", 50)
        confidence = min(1.0, 0.6 * min(1.0, inliers / max_inliers) + 0.4 * (inliers / max(total_matches, 1)))
    ...
    def _calculate_fov_polygon(
        self, cx: float, cy: float, half_w: float, half_h: float, global_angle: int, R_ref: np.ndarray
