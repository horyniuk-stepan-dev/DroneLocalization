...
def _graph_with_false_closure():
    ...
    opt.add_edge(1, 5, _sim(0.0, 0.0), weight=0.5, edge_type="spatial", inliers=16, rmse=5.0)
...
class TestTwoStagePrune:
    ...
    def test_never_disconnects_node_from_anchors(self):
        ...
        opt.add_node(7)
        opt.add_edge(
            3, 7, _sim(999.0, 999.0), weight=0.3, edge_type="spatial", inliers=16, rmse=6.0
        )
        opt.initialize_from_bfs()
        opt.optimize(
            max_iterations=60, tolerance=1e-10, use_analytic_jac=True, two_stage_prune=True
