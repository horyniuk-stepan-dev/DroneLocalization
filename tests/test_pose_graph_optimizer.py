...
class TestAffineStateConversion:
    """Тести конвертації affine ↔ state."""

    def test_roundtrip(self):
        M = _make_similarity(100.0, 200.0, 0.5, 30.0)
        state = _affine_to_state(M)
        M_back = _state_to_affine(state)
        np.testing.assert_allclose(M, M_back, atol=1e-5)

    def test_identity(self):
        M = _make_similarity(0.0, 0.0, 1.0, 0.0)
        state = _affine_to_state(M)
        assert abs(state[0]) < 1e-6  # tx
        assert abs(state[1]) < 1e-6  # ty
        assert abs(state[2]) < 1e-6  # log_s = log(1) = 0
        assert abs(state[3]) < 1e-6  # θ = 0
...
class TestPredictForwardInverse:
    """Тести прямої та зворотної передбачуваності."""

    def test_forward_identity_edge(self):
        """Тотожне ребро не змінює стан."""
        state_i = np.array([100.0, 200.0, np.log(0.5), 0.3])
        edge = GraphEdge(
            from_id=0,
            to_id=1,
            dtx=0,
    ...
    def test_forward_inverse_roundtrip(self):
        ...
        state_j = _predict_forward(state_i, edge)
        state_i_back = _predict_inverse(state_j, edge)
...
class TestLinearGraph:
    ...
    def test_linear_three_nodes(self):
        ...
        # Ребра: 0→1 з translation 100px, 1→2 з translation 100px
        ...
        # Вузол 1 має бути приблизно на tx=100 (середина між 0 і 200)
        M1 = results[1]
...
class TestLoopClosure:
    ...
    def test_ring_graph(self):
        ...
        # Кожне ребро: translation 100px вправо
        step = _make_similarity(100.0, 0.0, 1.0, 0.0)
        for i in range(4):
            opt.add_edge(i, i + 1, step, weight=1.0, edge_type="temporal")

        # Замикання: 4→0 (має бути translation -400 вліво, тобто повернення)
        ...
        # Перевіряємо що вузли розтягнуті рівномірно
        for i in range(5):
            M = results[i]
            tx, ty, s, a = decompose_affine(M)
            expected_tx = i * 100.0
            # З loop closure результат має бути точнішим
...
class TestSingleAnchor:
    ...
    def test_single_anchor_propagation(self):
        ...
        # Часові ребра: кожне з малою translation
        ...
        # Всі 10 вузлів мають отримати координати
...
class TestSO2Correctness:
    ...
    def test_angle_wrapping(self):
        ...
        # Ребра: невелике обертання на кожному кроці
...
class TestGeoJSONExport:
    ...
    def test_export_structure(self):
        ...
        opt.fix_node(0, M0)
        opt.add_node(1, _affine_to_state(M1))
        ...
        # Мають бути Point (вузли) та LineString (ребра)
